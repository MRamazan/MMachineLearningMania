...
mens_df_seeds = mens_df_seeds[mens_df_seeds["Season"] < 2024]
womens_df_seeds["Seed"] = womens_df_seeds["Seed"].str.extract("(\d+)").astype(int)
womens_df_seeds = womens_df_seeds[womens_df_seeds["Season"] < 2024]
...
mens_df_regular_compact = mens_df_regular_compact[mens_df_regular_compact["Season"] < 2024]
...
class MMarchManiaDataset(Dataset):
    def __init__(self):
        ...
        self.mens_df_seeds = mens_df_seeds[mens_df_seeds["Season"] < 2024]
        self.womens_df_seeds = womens_df_seeds[womens_df_seeds["Season"] < 2024]


        self.mens_df_regular_compact = load_csv(os.path.join(dirname, 'MRegularSeasonCompactResults.csv'))
        self.mens_df_regular_compact = mens_df_regular_compact[mens_df_regular_compact["Season"] < 2024]
        ...
        self.mcoaches["CoachID"] = encoder.fit_transform(self.mcoaches["CoachName"])

        self.games = pd.concat((self.mens_df_tourney_detailed, self.mens_df_regular_detailed))
    ...
    def get_coach(self,season, team_id):
        coach = self.mcoaches[(self.mcoaches["Season"] == season) & (self.mcoaches["TeamID"] == team_id)]["CoachName"]
        return coach.iloc[0] if not coach.empty else None
    ...
    def calculate_diffs(self, wseed, lseed, wwrate, lwrate, wavgscore, lavgscore, wavg_score_eaten, lavg_score_eaten,
                        ...
                        lwblk, wwpf, lwpf, wwftm, lwftm, t1coach, t2coach):
    ...
    def __getitem__(self, idx):
        ...
        prev_games = self.games.iloc[:idx]


        team1 = row["WTeamID"]
        team2 = row["LTeamID"]

        coach = self.mcoaches[self.mcoaches["TeamID"] == team1]

        season = row["Season"]
        coach = coach[coach["Season"] == season]

        team1_wgames = prev_games[prev_games["WTeamID"] == team1]
        team1_wgames = team1_wgames[team1_wgames["Season"] > min(2023,max(1985,season - 20))]

        team1_lgames = prev_games[prev_games["LTeamID"] == team1]
        team1_lgames = team1_lgames[team1_lgames["Season"] > min(2023,max(1985,season - 20))]

        team2_wgames = prev_games[prev_games["WTeamID"] == team2]
        team2_wgames = team2_wgames[team2_wgames["Season"] > min(2023,max(1985,season - 20))]

        team2_lgames = prev_games[prev_games["LTeamID"] == team2]
        team2_lgames = team2_lgames[team2_lgames["Season"] > min(2023,max(1985,season - 20))]
        ...
        team1_seeds = self.mens_df_seeds[self.mens_df_seeds["TeamID"] == team1]

        team1_avg_seed = team1_seeds[team1_seeds["Season"] < row["Season"]]
        team1_avg_seed = team1_avg_seed[team1_avg_seed["Season"] > (min(2023,max(1985,row["Season"] - 20)))]["Seed"].mean()


        team2_seeds = self.mens_df_seeds[self.mens_df_seeds["TeamID"] == team2]

        team2_avg_seed = team2_seeds[team2_seeds["Season"] < row["Season"]]
        team2_avg_seed = team2_avg_seed[team2_avg_seed["Season"] > min(2023,max(1985,row["Season"] - 20))]["Seed"].mean()
        ...
        output = [team1_avg_score.mean(), team2_avg_score.mean(), team1_avg_eaten_score.mean(), team2_avg_score_eaten.mean()]
        nan = False
        for o in output:
            if np.isnan(o):
                nan = True
        ...
        return [team1_avg_score, team2_avg_score, team1_avg_seed, team2_avg_seed]

x = [[0,2,3], [5,8,9],[87,9,8]]
seeds = np.load("seeds.npy")
seeds = seeds[~np.isnan(seeds)]
print(np.mean(seeds))
print(np.nanmedian(seeds))

dataset = MMarchManiaDataset()

list_ = []
seed_nan = 0
print(np.array(dataset.labels).shape)
...
np.save("data.npy",np.array(dataset.data))
np.save("labels.npy",np.array(dataset.labels))

print(np.array(dataset.labels).shape)
print(np.array(dataset.data).shape)
print(sum(np.array(list_)[:,2]) / len(list_))
print(sum(np.array(list_)[:,3]) / len(list_))
print(len(list_))
